  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-1 rounded-full bg-white/5 p-1 ring-1 ring-inset ring-white/10 backdrop-blur lg:flex">
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="rounded-full px-4 py-2 text-sm font-medium text-slate-200 transition-colors hover:bg-white/10 hover:text-white"
            >
              {link.label}
            </a>
          ))}
        ...
        <a
          href="#contacto"
          className="hidden rounded-full bg-ember-400 px-5 py-2.5 text-sm font-semibold text-fenixNavy-950 shadow-sm shadow-ember-900/30 transition-colors hover:bg-ember-300 lg:inline-block"
        >
          Consultanos
        </a>
